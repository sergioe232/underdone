...
function GM:HUDPaint()
	...
	if LocalPlayer():IsMelee() or !ValidEntity(LocalPlayer():GetActiveWeapon()) then
	...
	self:DrawHealthBar()
	self:DrawLevelBar()
	if !LocalPlayer():IsMelee() && ValidEntity(LocalPlayer():GetActiveWeapon()) then self:DrawAmmoBar() end		
			...
			for strItem, intAmountNeeded in pairs(tblQuestTable.ObtainItems or {}) do
				...
				if intItemsGot < intAmountNeeded then
					...
					for _,prop in pairs(ents.FindByClass("prop_physics")) do
						if ValidEntity(prop) && tblItemTable.Model == prop:GetModel() then
...
function GM:DrawSquadMembers(intYOffset, intDirection)
	...
	for key, plySquadMate in pairs(LocalPlayer().Squad or {}) do
		if !ValidEntity(plySquadMate) then LocalPlayer().Squad[key] = nil end
		if ValidEntity(plySquadMate) && plySquadMate != LocalPlayer() then
