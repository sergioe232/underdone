...
function Player:GetMasterExp(strMaster)
	if !ValidEntity(self) or !self.Data.Masters then return 0 end
...
function Player:GetMasterLevel(strMaster)
	if !ValidEntity(self) or !self.Data.Masters then return 1 end
...
function Player:GetTotalMasters()
	if !ValidEntity(self) or !self.Data.Masters then return #GAMEMODE.DataBase.Masters end
	local intTotal = 0
...
if SERVER then
	function Player:BuyMasterLevel(strMaster)
		if !ValidEntity(self) or !self.Data.Masters then return false end
