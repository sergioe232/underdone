...
if SERVER then
	function GM:AddAuction(plySeller, strItem, intAmount, intSellPrice)
		if !ValidEntity(plySeller) or !plySeller:HasItem(strItem, intAmount) then return false end
	...
	function GM:CancelAuction(plyCanceler, intKey)
		if !ValidEntity(plyCanceler) or !GAMEMODE.Auctions[intKey] then return false end
	...
	function GM:PickUpAuction(plyPickerUper, intKey)
		if !ValidEntity(plyPickerUper) or !GAMEMODE.Auctions[intKey] then return false end
	...
	function GM:BuyOutAuction(plyBuyer, intKey)
		if !ValidEntity(plyBuyer) or !GAMEMODE.Auctions[intKey] then return false end
	...
	function GM:LoadAuctions()
		...
		if file.Exists(strFileName) then
			GAMEMODE.Auctions = glon.decode(file.Read(strFileName))
		end
	end
	...
	concommand.Add("UD_RequestAuctionInfo", function(ply, command, args)
		if args[1] then
			...
			for intKey, tblInfo in pairs(GAMEMODE.Auctions) do
				if (intCounter >= (ply:GetNWInt("AuctionPage") * GAMEMODE.AuctionsPerPage) && intCounter < ((ply:GetNWInt("AuctionPage") + 1) * GAMEMODE.AuctionsPerPage)) or ply:SteamID() == tblInfo.SellerID or SinglePlayer()  then
					if tblInfo.TimeLeft > intCleanUpTime - intDefaultAuctionTime or ply:SteamID() == tblInfo.SellerID or SinglePlayer() then
